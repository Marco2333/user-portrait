...
# 生成某一用户的XML文件
def generate_user_xml(twitter_user):
	...
	if twitter_user.has_key('_id'):
		id_text = dom.createTextNode(twitter_user['_id'])
	else:
		id_text = dom.createTextNode(twitter_user['user_id'])
	...
	# 姓名标签内容,将编码方式转换一下
	name_text = dom.createTextNode(twitter_user['name'])
	# 将内容加入标签中
	name_ele.appendChild(name_text)

	# screen_name 节点
	sn_ele = dom.createElement('screen_name')
	sn_text = dom.createTextNode(twitter_user['screen_name'])
	sn_ele.appendChild(sn_text)

	# id 节点
	location_ele = dom.createElement('地理位置')
	if twitter_user['location'] != "":
		location_text = dom.createTextNode(twitter_user['location'])
	else:
	...
	category_text = dom.createTextNode(twitter_user['category'])
	...
	interest_text = dom.createTextNode(twitter_user['interest_tags'])
	interest_ele.appendChild(interest_text)

	# 用户社交影响力标签
	influence_ele = dom.createElement("影响力分数")
	influence_text = dom.createTextNode(str(twitter_user['influence_score']))
	influence_ele.appendChild(influence_text)

	activity_ele = dom.createElement("活跃度")
	activity_text = dom.createTextNode(str(twitter_user['activity']))
	activity_ele.appendChild(activity_text)

	if twitter_user['influence_score'] >= 110:
		influence_rank = '高'
	elif twitter_user['influence_score'] >= 60:
		influence_rank = '中'
	else:
		influence_rank = '低'
	# 用户社交影响力大小
	...
	influence_rank.appendChild(influence_rank_text)

	# 将隐性属性标签加入到隐性标签中
	implicit_info.appendChild(category_ele)
	implicit_info.appendChild(FeelingE)
	...
	implicit_info.appendChild(NewTweetE)
